...
function listarReporteRstFrm() {
    ...
    params.append("accion", "reporte_rst_frm_teo");
    params.append("cod_emp", 1);
...
function construirTablaDias(data) {
    ...
    try {
        ...
        asesores.forEach(a => {
            html += `<th>${a}</th>`; // 🔹 Solo 1 columna por asesor
        });

        html += `<th rowspan="2">Total</th></tr><tr>`;

        asesores.forEach(() => {
            html += `<th>WhatsApp</th>`; // 🔹 Solo WhatsApp
        });

        html += `</tr></thead><tbody>`;

        let totalesAsesor = {};
        asesores.forEach(a => totalesAsesor[a] = { WhatsApp: 0 });
